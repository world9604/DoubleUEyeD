...
public class TensorFlowClassifier {
    ...
    private static final String MODEL_FILE = "file:///android_asset/model_graph_eq.pb";
...
//        tii.feed("left/input_scope/is_training", new boolean[]{false}, 1);
        ...
        float[] result = {0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f};
        ...
        for (int num = 0; num < bundleSize; num++) {
            ...
            for (int i = 0; i < MULTISCALE_CNT; i++) {
                ...
                if (i == 0) {
                    ...
                    lowLeftData = grayScale_Equalization_Norm(tmpLeftBitmap);
                } else if (i == 1) {
                    midRightData = grayScale_Equalization_Norm(tmpRightBitmap);
                    midLeftData = grayScale_Equalization_Norm(tmpLeftBitmap);
                } else {
                    highRightData = grayScale_Equalization_Norm(tmpRightBitmap);
                    highLeftData = grayScale_Equalization_Norm(tmpLeftBitmap);
                }
            }
     ...
     * @return
     */
    private static float[] grayScale_Equalization_Norm (Bitmap bitmap) {

        Mat matbit = new Mat(bitmap.getHeight(), bitmap.getWidth(), CV_8UC1);  //CV_32F   CV_32FC3 (CV_32SC3: 32bit-float)  CV_8UC4     // row
        Mat matGray = new Mat();
        ...
        // 3. Normalization
        int mWidth = matbit.width();    // cols
        int mHeight = matbit.height();  // row

        float[] norm_pixels = new float[mWidth * mHeight];
        int[] ori_pixels = new int[mWidth * mHeight];

        for(int row=0; row<matbit.height(); row++){      // row
            for(int col=0; col<matbit.width(); col++) {  // col

                int grayPixel = (int) (matGrayEquli.get(row , col)[0]);

                int i = col + row * matbit.width();

                ori_pixels[i] = grayPixel;
